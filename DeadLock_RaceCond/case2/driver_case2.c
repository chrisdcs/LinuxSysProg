...
int e2_open(struct inode *inode, struct file *filp)
{
    ...
    if (devc->mode == MODE1) {
        ...
        pr_info("open device No. %d in Mode 1\n", devc->count1);
        return 0;
    }
    else if (devc->mode == MODE2) {
        devc->count2++;
        pr_info("open device No. %d in Mode 2\n", devc->count2);
    }
    up(&devc->sem1);
    return 0;
}
...
int e2_release(struct inode *inode, struct file *filp)
{
    ...
    if (devc->mode == MODE1) {
        devc->count1--;
        pr_info("release device No. %d in Mode 1\n", devc->count1);
        if (devc->count1 == 1){
            wake_up_interruptible(&(devc->queue1));
            pr_info("wake up in Mode 1 when count1 == 1\n");
        }
			up(&devc->sem2);
    }
    else if (devc->mode == MODE2) {
        devc->count2--;
        pr_info("release device No. %d in Mode 2\n", devc->count2);
        if (devc->count2 == 1){
            pr_info("wake up in Mode 2 when count2 == 1\n");
...
static ssize_t e2_read (struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (devc->mode == MODE1) {
	   up(&devc->sem1);
       pr_info("Device No. %d read in Mode 1 \n",devc->count1);
	   ...
	   ret = count - copy_to_user(buf, devc->ramdisk, count);
        ...
        pr_info("Device No. %d read in Mode 2 \n",devc->count2);
          if (*f_pos + count > ramdisk_size) {
             printk("Trying to read past end of buffer!\n");
             up(&devc->sem1);
             return ret;
...
static ssize_t e2_write (struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	down_interruptible(&devc->sem1);
	if (devc->mode == MODE1) {
        pr_info("Device No. %d write in Mode 1 \n",devc->count1);
		up(&devc->sem1);
        if (*f_pos + count > ramdisk_size) {
            printk("Trying to read past end of buffer!\n");
            return ret;
        ...
        pr_info("Device No. %d write in Mode 2 \n",devc->count2);
        if (*f_pos + count > ramdisk_size) {
            printk("Trying to read past end of buffer!\n");
            up(&devc->sem1);
            return ret;
...
static long e2_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		case E2_IOCMODE2:
                pr_info("changing Mode 1 to Mode 2\n");
				down_interruptible(&(devc->sem1));
				if (devc->mode == MODE2) {
					up(&devc->sem1);
					break;
				...
				if (devc->count1 > 1) {
					while (devc->count1 > 1) {
						up(&devc->sem1);
                        pr_info("wait until only 1 device in Mode 1 exists\n");
					    wait_event_interruptible(devc->queue1, (devc->count1 == 1));
						down_interruptible(&devc->sem1);
					}
				}
				devc->mode = MODE2;
                pr_info("change Mode 1 to Mode 2 successful\n");
                devc->count1--;
                devc->count2++;
				up(&devc->sem2);
				up(&devc->sem1);
		...
		case E2_IOCMODE1:
                pr_info("changing Mode 2 to Mode 1\n");
				down_interruptible(&devc->sem1);
				if (devc->mode == MODE1) {
				   up(&devc->sem1);
				   break;
				...
				if (devc->count2 > 1) {
				   while (devc->count2 > 1) {
				       up(&devc->sem1);
                       pr_info("wait until only 1 device in Mode 2 exists\n");
				       wait_event_interruptible(devc->queue2, (devc->count2 == 1));
				       down_interruptible(&devc->sem1);
				   }
				}
				devc->mode = MODE1;
                pr_info("change Mode 2 to Mode 1 successful\n");
                devc->count2--;
                devc->count1++;
				down_interruptible(&devc->sem2);
				up(&devc->sem1);
